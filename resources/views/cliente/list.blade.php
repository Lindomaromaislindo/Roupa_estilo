...
<div class="card shadow p-4 mb-4 border-success">
    ...
    <form action="{{ route('cliente.search') }}" method="post" class="row g-3 mb-4">
        ...
        <div class="col-md-3 d-flex align-items-end">
            ...
            <a href="{{ url('cliente/create') }}" class="btn btn-outline-success me-2">Novo</a>
            <a href="{{ route('cliente.pdf') }}" target="_blank" class="btn btn-outline-primary">Gerar PDF</a>
        </div>
    </form>
                ...
                @forelse ($dados as $item)
                    <tr>
                        ...
                        <td class="text-center">
                            @if (!empty($item->imagem) && file_exists(public_path('storage/' . $item->imagem)))
                                <img
                                    src="{{ asset('storage/' . $item->imagem) }}"
                                    alt="Imagem do cliente"
                                    width="100"
                                    height="100"
                                    class="border mx-auto d-block"
                                >
                            @else
